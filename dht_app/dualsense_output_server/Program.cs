...
while (true)
{
    ...
    else if (IsTriggerReset(message))
    {
        ...
        if (triggerWriter.Connected)
        {
            lock (triggerStateLock)
            {
                ...
                currentBrakeVibrateAmp = 0;
                currentBrakeVibrateFreq = 0;
                currentBrakeVibrateStartZone = 0;
                ...
                currentGearShiftKickSide = 0;
                currentGearShiftKickForce = 0;
                currentGearShiftKickStart = -1;
                currentGearShiftKickStartedUtc = DateTime.MinValue;
                currentGearShiftKickUntilUtc = DateTime.MinValue;
                currentGearShiftKickReleaseForce = 0;
                currentGearShiftKickReleaseDurationMs = 45;
                currentGearShiftKickSoftness = 7;
                triggerLoopPausedUntilUtc = DateTime.MinValue;
                lastTriggerUpdateUtc = DateTime.UtcNow;
            }
            lock (triggerWriteLock)
            {
                triggerWriter.ResetTriggers();
            }
            Console.WriteLine($"{DateTime.Now:HH:mm:ss.fff} TRIGGER_RESET off");
        }
    }
...
static bool IsTriggerReset(string message)
{
    var parts = message.Split('|', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
    return parts.Length > 0 && (
        string.Equals(parts[0], "TRIGGER_RESET", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(parts[0], "TRIGGER_OFF", StringComparison.OrdinalIgnoreCase));
}
